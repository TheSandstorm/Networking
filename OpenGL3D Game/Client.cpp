...
// Description	: Client side of the chat room
// Author		: John Strange
...
bool CClient::BroadcastForServers()
{
	...
	TPacket _packet;
	_packet.Serialize(BROADCAST, const_cast<char*>("Broadcast to Detect Server"));
...
void CClient::ProcessData(char* _pcDataReceived)
{
	...
	case DATA:
	{
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 10);
		std::cout << "SERVER> " << _packetRecvd.MessageContent << std::endl;
		if (_packetRecvd.MessageContent == "You have left the server")
		{
	...
	{
		TPacket _packet;
		_packet.Serialize(KEEPALIVES, const_cast<char*>("Alive"));
		SendData(_packet.PacketData);
		break;
	}
	case SHUTDOWN:
...
bool CClient::SelectServer(int _Opt) 
{
	...
	TPacket _packet;
	_packet.Serialize(HANDSHAKE, const_cast<char*>("Hello"));
	SendData(_packet.PacketData);
	return true;
}